...
const CategorySingleCard = ({ laptopItem, setProductData, handleReportedProduct }) => {
    const { name, category, categoryImage, image, location, yearsOfUse, resalePrice, originalPrice, postedTime, seller, condition, mobile, verified, description, buyerReported } = laptopItem;
            ...
            <div className='space-y-4 p-5 w-full'>
                ...
                <div className='flex flex-col lg:flex-row lg:justify-between gap-8 text-primary w-full'>
                    ...
                    <div className='font-semibold flex flex-col gap-2 w-full lg:w-1/2 sm:text-left text-center items-center sm:items-start'>
                        ...
                        <button onClick={() => handleReportedProduct(laptopItem)} disabled={buyerReported} className='btn h-auto min-h-0 bg-red-800 hover:bg-primary border-0 rounded capitalize py-2 text-white'>
                            
                            {
                                buyerReported? 'Reported' : 'Report The Product'
                            }
                        </button>
                    </div>
                </div>
